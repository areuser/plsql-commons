declare
     fileLoc      type.string := 'CRUD';
     lTemplate StringList := StringList ();
     lContext StringList := StringList ();
     lOutput StringList := StringList (); 
     tplName type.string := 'emp_adr';
     column_list1 type.string;
     column_list2 type.string;
     column_list3 type.string;
...
begin 
     ...
     for r2 in c2 loop  
       column_list1 := column_list1 || pUtil.printf ('    rLog.{1} := :new.{1};
      ', varchars_t(r2.column_name)); 
       column_list2 := column_list2 || pUtil.printf ('  , {1}
      ', varchars_t(r2.column_name));
       column_list3 := column_list3 || pUtil.printf ('  , :old.{1}
      ', varchars_t(r2.column_name)); 
     end loop;   
     ...
     lContext := plsql_gen.addContext (lContext, 'column_list3', column_list3);
